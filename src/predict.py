import numpy as np
from scipy.ndimage import binary_dilation, binary_fill_holes, gaussian_filter, binary_erosion, distance_transform_edt, \
...
def _reinterpolate_bad_components(
    ...
    for iteration in range(max_iterations):
        ...
        # ── Napari debug visualization ─────────────────────────────────────────
        # _napari_debug_reinterp(
    ...
    # ── Final beta1 tally ─────────────────────────────────────────────────────
    # final_binary = result_labeled > 0
    # final_labeled = label(final_binary)
    # n_final = final_labeled.max()
    # still_bad = sum(
    #     (1 - euler_number((final_labeled == cid).astype(int), connectivity=1)) > 0
    #     for cid in range(1, n_final + 1)
    # )
    # print(f"\n  After re-interpolation: {still_bad}/{n_final} components still have β1>0")
...
def run_inference(args):
    ...
    if args.checkpoints is None:
        checkpoints = ["checkpoint_final.pth"] * n_models
    elif len(args.checkpoints) == 1:
        checkpoints = args.checkpoints * n_models
    else:
        assert len(args.checkpoints) == n_models

    # --- devices ---
    if args.devices is None:
        devices = [f"cuda:{i % torch.cuda.device_count()}" for i in range(n_models)]
    elif len(args.devices) == 1:
        devices = args.devices * n_models
    else:
        assert len(args.devices) == n_models
    ...
    for model_path, ckpt, device in zip(args.models, checkpoints, devices):
        ...
        predictors.append(ParallelPredictor(predictor, device))

    # 5. Setup Image Reader
    reader = Tiff3DIO()

    # 6. Prepare Input Files
    test_files = sorted(glob.glob(os.path.join(input_dir, "*.tif")))
    if not test_files:
    ...
    with zipfile.ZipFile(CONFIG["output_zip"], 'w', zipfile.ZIP_DEFLATED) as zf:
        ...
        for file_path in tqdm(test_files, desc="Parallel Dual-GPU Inference"):
            ...
            with ThreadPoolExecutor(max_workers=2) as executor:
                ...
                future1 = executor.submit(predictors[0].predict, image, properties)
                future2 = executor.submit(predictors[1].predict, image, properties)
...
def parse_args():
    parser = argparse.ArgumentParser("nnUNet Ensemble Inference")

    parser.add_argument(
        "--models",
        nargs="+",
        required=True,
        help="Paths to trained nnUNet model folders"
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Checkpoint names (same length as models or single value)"
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="CUDA devices per model (e.g. cuda:0 cuda:1)"
    )
